...
function ThreadCard ({
    ...
    return (
        <article className={`flex w-full flex-col rounded-xl ${isComment ? 'px-0 xs:px-7' : 'bg-dark-2 p-7'} `}>
            <div className="flex items-start justify-between">
                <div className="flex w-full flex-1 flex-row gap-4">
                    <div className="flex flex-col items-center">
                        <Link aria-label="Thread author profile page" href={`/profile/${author.id}`} className="relative h-11 w-11">
                    ...
                    <div className="flex w-full flex-col">
                        <Link aria-label="Thread author profile page" href={`/profile/${author.id}`} className="w-fit">
                        ...
                        <div className={`${isComment && 'mb-10'} mt-5 flex flex-col gap-3 `}>
                            <div className="flex gap-3.5">
                                ...
                                <Link aria-label="Comment this thread" href={`/thread/${id}`}>
            ...
            {!isComment && community && (
                <Link aria-label="Community page" href={`/communities/${community.id}`} className="mt-5 flex items-center">
